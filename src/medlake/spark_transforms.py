...
def normalize_events(df):
    ...
    return (
        df.withColumn("event_ts", F.try_to_timestamp("event_ts"))
        .withColumn("model_confidence", F.expr("try_cast(model_confidence AS DOUBLE)"))
        .withColumn("image_quality_score", F.expr("try_cast(image_quality_score AS DOUBLE)"))
        .withColumn("schema_version", F.expr("try_cast(schema_version AS INT)"))
        .withColumn("referred", F.expr("try_cast(referred AS BOOLEAN)"))
